...
if ( ! class_exists( __NAMESPACE__ . '\Settings_Renderer' ) ) {
	...
	final class Settings_Renderer extends Meta_Settings_Renderer {
		...
		public function render_settings_page_sidebar() {
			...
			$image_lnk = Lib::safe_key_value( $pluginbox, 'logo', $this->plugin->get_url_path() . '\/assets/' . $this->pageslug . '-logo.png' );
			$click_lnk = Lib::safe_key_value( $pluginbox, 'link', 'https://wordpress.org/plugins/' . $this->pageslug . '/' );
