...
if ( ! class_exists( __NAMESPACE__ . '\\Plugin' ) ) {
	...
	# region Main Properties
		 ...
		 * 'intermediate_image_sizes_advanced' filter and
		 * used in 'wp_generate_attachment_metadata' filter.
		 *
		 * @var bool|array $my_metadata stored.
		 */
		private $my_metadata = false;
	...
	# region Upload vs Regenerate Detection & handling Methods.
		 ...
		 * Use flag to find out wether plugin is compressing
...
<style type="text/css">#regenerate-thumbnails-app div:first-child div:first-child p:nth-of-type(2) label{display:none!important}</style>
	...
	# region Image Upload & Regenerate Hooks
		...
		public function on_intermediate_image_sizes_advanced_99_filter( $sizes, $metadata = false, $attachment_id = false ) {
			...
			if ( $post_mime_type !== $image_mime_type ) {
				Lib::error( 'Post mime-type doesn\'t match image mime-type :' . $post_mime_type . '.' );
				return $sizes;
			}
			...
			$track_files = array();
			$track_saved = get_post_meta( $attachment_id, '_warp_imagick_files', $single = true );
			if ( is_array( $track_saved ) ) {
				$track_files = $track_saved;
			} else {
				update_post_meta( $attachment_id, '_warp_imagick_files', $track_files );
			}

			if ( ! empty( $metadata['file'] ) ) {
				$track_files[ wp_basename( $metadata['file'] ) ] = null;
			}
			if ( ! empty( $metadata['original_image'] ) ) {
				$track_files[ wp_basename( $metadata['original_image'] ) ] = null;
			}
			...
			if ( $is_regenerate ) {
				...
				if ( ! empty( $old_metadata['file'] ) ) {
					$track_files[ wp_basename( $old_metadata['file'] ) ] = null;
				}
				if ( ! empty( $old_metadata['original_image'] ) ) {
					$track_files[ wp_basename( $old_metadata['original_image'] ) ] = null;
				}
				foreach ( $old_metadata['sizes'] as $size_name => $size_data ) {
					if ( ! empty( $size_data['file'] ) ) {
						$track_files[ wp_basename( $size_data['file'] ) ] = null;
					}
				}
				if ( is_array( $backup_sizes ) ) {
					foreach ( $backup_sizes as $backup_size ) {
						if ( ! empty( $backup_size['file'] ) ) {
							$track_files[ wp_basename( $backup_size['file'] ) ] = null;
						}
					}
				}
			...
			if ( empty( $warp_original ) ) {
				...
				if ( ! empty( $warp_original ) ) {
					...
					foreach ( $track_files as $basename => $flag ) {
					...
					if ( Shared::is_edited( $warp_original ) && 'edited' !== $this->my_image_state ) {
						...
						Lib::debug( 'Warp original is uploaded as user-edited regex-pattern matching filename?: ' . $warp_original );
					}

					update_post_meta( $attachment_id, '_warp_imagick_source', $warp_original );

					Lib::debug( 'Warp original saved: ' . $warp_original );
			...
			if ( ! empty( $metadata['original_image'] ) ) {
				$track_files[ $metadata['original_image'] ] = null;
			}
			...
			switch ( $this->do_generate_webp_clones() ) {
				case false:
				case 0:
					foreach ( $track_files as $collected_name => $ignore ) {
						$temp_path = \path_join( dirname( $metadata['file'] ), $collected_name );
						$temp_path = $this->get_absolute_upload_file_path( $temp_path );
						$webp_path = Shared::get_webp_file_name( $temp_path );
	...
	# region Helper functions
		 ...
		 */
		public function webp_clone_image( $image_path, $mime_type = '' ) {
			...
			switch ( $mime_type ) {
				...
				case 'image/png':
					...
					if ( function_exists( 'imagepalettetotruecolor' ) ) {
						if ( \imagepalettetotruecolor( $gd_png ) ) {
							$gd_image = $gd_png;
							$gd_png   = false;
							break;
						}
			...
			if ( false === $gd_image ) {
				...
				if ( \imagewebp( $gd_image, $webp_path, $webp_quality ) ) {
					if ( \file_exists( $webp_path ) ) {
