...
require HERMIT_PATH . '/Meting.php';
...
class HermitJson
{
	...
	{
		$cache_key = "/netease_v3/song/$music_id";
		...
		if ($response["code"] == 200 && $response["songs"]) {
			//处理音乐信息
			$mp3_url    = json_decode($this->meting->format()->url($music_id), true);
			$mp3_url    = $mp3_url['url'];
	...
	{
		$key = "/netease_v3/playlist/$playlist_id";
		...
		if ($response["code"] == 200 && $response["result"]) {
			...
			foreach ($result as $k => $value) {
				$mp3_url    = json_decode($this->meting->format()->url($value["id"]), true);
				$mp3_url    = $mp3_url['url'];
	...
	{
		$key = "/netease_v3/radios/$radio_id";
		...
		if ($response["code"] == 200 && $response["programs"]) {
			...
			foreach ($result as $k => $val) {
				$mp3_url    = json_decode($this->meting->format()->url($val['mainSong']['id']), true);
				$mp3_url    = $mp3_url['url'];
