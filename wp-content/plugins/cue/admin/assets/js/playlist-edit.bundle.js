...
var Tracks = backbone__WEBPACK_IMPORTED_MODULE_0___default.a.Collection.extend({
...
var Track = backbone__WEBPACK_IMPORTED_MODULE_0___default.a.Model.extend({
  defaults: {
    artist: '',
    artworkId: '',
    artworkUrl: '',
...
var Attachment = wp__WEBPACK_IMPORTED_MODULE_1___default.a.media.model.Attachment;
var l10n = cue__WEBPACK_IMPORTED_MODULE_2__["default"].l10n;
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  get: function get(id) {
    var method = '_' + id;
    var frame = this.frames[method] || null; // Always call the frame method to perform any routine set up. The
  ...
  _selectArtwork: function _selectArtwork(frame) {
    var workflow = this; // Return existing frame for this workflow.
    ...
    frame.on('open', function () {
      ...
      var artworkId = workflow.model.get('artworkId');
      var attachments = [];
  ...
  _selectAudio: function _selectAudio(frame) {
    var workflow = this; // Return the existing frame for this workflow.
    ...
    frame.on('open', function () {
      ...
      var audioId = workflow.model.get('audioId');
      var audioUrl = workflow.model.get('audioUrl');
      var isEmbed = audioUrl && !audioId;
      var attachments = []; // Automatically select the existing audio file if possible.
    ...
    frame.state('insert').on('insert', function (selection) {
      ...
      var keys = underscore__WEBPACK_IMPORTED_MODULE_0___default.a.keys(workflow.model.attributes); // Attributes that shouldn't be updated when inserting an
      // audio attachment.
...
jquery__WEBPACK_IMPORTED_MODULE_0___default()(function ($) {
  var tracks = new cue__WEBPACK_IMPORTED_MODULE_2__["default"].model.Tracks(cue__WEBPACK_IMPORTED_MODULE_2__["default"].data.tracks);
  cue__WEBPACK_IMPORTED_MODULE_2__["default"].tracks = tracks;
  delete cue__WEBPACK_IMPORTED_MODULE_2__["default"].data.tracks;
  var screen = new _views_screen_edit_playlist__WEBPACK_IMPORTED_MODULE_3__["EditPlaylistScreen"]({
    el: '#post',
    collection: tracks,
...
var TrackList = wp__WEBPACK_IMPORTED_MODULE_2___default.a.Backbone.View.extend({
  ...
  addTrack: function addTrack(track) {
    var trackView = new _track__WEBPACK_IMPORTED_MODULE_3__["Track"]({
      model: track
    });
  ...
  updateOrder: function updateOrder() {
    ...
    underscore__WEBPACK_IMPORTED_MODULE_1___default.a.each(this.$el.find('.cue-track'), function (item, i) {
      var cid = jquery__WEBPACK_IMPORTED_MODULE_0___default()(item).data('cid');

      _this2.collection.get(cid).set('order', i);
    });
  }
});
...
var TrackArtwork = wp__WEBPACK_IMPORTED_MODULE_1___default.a.Backbone.View.extend({
  tagName: 'span',
...
var TrackAudio = wp__WEBPACK_IMPORTED_MODULE_2___default.a.Backbone.View.extend({
  tagName: 'span',
  ...
  render: function render() {
    ...
    var playerId = this.$el.find('.mejs-audio').attr('id'); // Remove the MediaElement player object if the
    ...
    var $mediaEl = this.$el.find('.cue-audio');
    ...
    if ($mediaEl.length) {
      ...
      var playerSettings = {
        ...
        success: function success(mediaElement, domObject, t) {
          var $fakeBody = jquery__WEBPACK_IMPORTED_MODULE_0___default()(t.container).parent(); // Remove the fake <body> tag.
        ...
        error: function error(el) {
          ...
          var $parent = $el.closest('.cue-track');
          var playerId = $el.closest('.mejs-audio').attr('id'); // Remove the audio element if there is an error.
  ...
  refresh: function refresh(e) {
    ...
    var player = playerId ? mediaelementjs__WEBPACK_IMPORTED_MODULE_1___default.a.players[playerId] : null;
...
var Track = wp__WEBPACK_IMPORTED_MODULE_1___default.a.Backbone.View.extend({
  tagName: 'li',
  className: 'cue-track',
  ...
  updateAttribute: function updateAttribute(e) {
    var attribute = jquery__WEBPACK_IMPORTED_MODULE_0___default()(e.target).data('setting');
  ...
  updateFields: function updateFields() {
    var track = this.model.toJSON();
    var $settings = this.$el.find('[data-setting]'); // A change event shouldn't be triggered here, so it won't cause
    // the model attribute to be updated and get stuck in an
    // infinite loop.

    for (var attribute in track) {
      var value = jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div/>').html(track[attribute]).text(); // Decodes HTML entities.
...
var _wp$media$view = wp__WEBPACK_IMPORTED_MODULE_0___default.a.media.view,
    MediaFrame = _wp$media$view.MediaFrame,
    Toolbar = _wp$media$view.Toolbar;
var PostFrame = MediaFrame.Post,
    SelectFrame = MediaFrame.Select;
var TrackMediaFrame = PostFrame.extend({
...
var AddTracksButton = wp__WEBPACK_IMPORTED_MODULE_1___default.a.Backbone.View.extend({
  id: 'add-tracks',
  tagName: 'p',
  events: {
    'click .button': 'click'
  ...
  render: function render() {
    var $button = jquery__WEBPACK_IMPORTED_MODULE_0___default()('<a />', {
...
var EditPlaylistScreen = wp__WEBPACK_IMPORTED_MODULE_1___default.a.Backbone.View.extend({
  saved: false,
  events: {
  ...
  buttonClick: function buttonClick(e) {
    ...
    var $button = jquery__WEBPACK_IMPORTED_MODULE_0___default()(e.target);
