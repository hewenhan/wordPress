...
if ( ! function_exists( 'themotion_contact_top' ) ) :
	...
	function themotion_contact_top() {
		...
		if ( ! empty( $themotion_contact_button_link ) && strpos( $themotion_contact_button_link, '#' ) === 0 ) {
			$themotion_go_to = 'href="#" onclick="return false;" data-anchor="' . esc_attr( $themotion_contact_button_link ) . '"';
		} else {
...
if ( ! function_exists( 'themotion_post_info' ) ) {
	...
	function themotion_post_info( $type, $post_id ) {
		...
		if ( ! empty( $array_list ) ) { ?>
			<div class="<?php if ( ( 'categories' == $type ) || ( 'download_categories' == $type ) ) { echo 'categories-links';
} elseif ( ( 'tags' == $type ) || ( 'download_tags' == $type ) ) { echo 'tags-links'; } ?>">
				<?php
				$i = 0;
				...
				if ( ! empty( $array_list ) ) {
					foreach ( $array_list as $item ) {
						...
						if ( $i < 2 || $i > 2 ) { ?>
							<a href="<?php echo esc_url( $label_link ); ?>" rel="<?php if ( 'categories' == $type ) {  echo 'category';
} else { echo 'tag'; } ?>">
								<?php
